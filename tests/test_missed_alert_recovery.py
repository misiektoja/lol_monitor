...
def test_a_channel_that_missed_the_failure_alert_is_told_about_the_whole_outage(monkeypatch):
    ...
    monitor.send_outage_recovery_alert("Watched#EUNE", 600, state)

    assert len(channels.calls) == 1
    call = channels.calls[0]
    assert (call["email"], call["webhook"]) == (True, True)
    assert call["body"].startswith("Monitoring recovered for Watched#EUNE after 10 minutes.")
    assert call["webhook_body"].startswith("Monitoring failed for Watched#EUNE at ")
