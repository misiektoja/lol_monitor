...
def runtime_command(command, prefix=None):
    module = importlib.import_module("lol_monitor")
    assert module.__file__ is not None
    manual = [sys.executable, str(Path(module.__file__).resolve())]
    packaged = [sys.executable, "-m", "lol_monitor"]
    ...
    command = re.sub(r"(?<![\w/])python(?:3(?:\.\d+)?)? lol_monitor\.py", lambda match: manual_prefix, command)
    return re.sub(r"(?<![\w/])lol_monitor(?= |$)", lambda match: packaged_prefix, command)
