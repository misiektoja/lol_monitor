...
# Reloading saved answers cannot mutate the cached template used by later saves
def test_saved_reload_leaves_template_defaults_unchanged(tmp_path):
    defaults = dict(monitor._config_template_defaults())
    config = tmp_path / "settings.conf"
    config.write_text('RIOT_ID = "Someone#EUW"\nREGION = "euw1"\n', encoding="utf-8")
    state = monitor.WizardSetupState(config, tmp_path / "private.env", {})
    monitor._wizard_apply_saved_values(state)
    assert monitor._config_template_defaults() == defaults
