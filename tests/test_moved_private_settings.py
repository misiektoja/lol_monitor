...
# Reports malformed routing maps before they can reach the region lookup
@pytest.mark.parametrize("routing", [17, [], {}, {"eun1": -1}, {"eun1": ["europe"]}])
def test_malformed_region_maps_remain_diagnosable(monkeypatch, routing):
    monkeypatch.setattr(monitor, "REGION_TO_CONTINENT", routing)
