...
monitor = importlib.import_module("lol_monitor")
...
def test_display_prefix_is_short(monkeypatch, host, executable, method):
    ...
    monkeypatch.setattr(sys, "argv", ["/private/long/tools/lol_monitor.py" if method == "manual" else "/private/long/bin/lol_monitor"])
    monkeypatch.setattr(monitor, "__file__", "/private/long/tools/lol_monitor.py")
    ...
    expected = executable + " lol_monitor.py --setup" if method == "manual" else "lol_monitor --setup"
...
def test_help_does_not_display_runtime_paths():
    ...
    assert "lol_monitor.py --setup" in result.stdout
