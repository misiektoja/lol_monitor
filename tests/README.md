...
These tests never touch the network or the Riot API. They assert the repository's own metadata, so a
change that breaks the citation, the sponsor button, the declared editor style or the release
integrity of a published archive fails before it reaches a release.
