...
# The optional wcwidth library measures wide characters correctly, which otherwise count as one column
...
COMMAND_PLACEHOLDERS = frozenset((RIOT_ID_PLACEHOLDER, REGION_PLACEHOLDER, "<new-file>"))
...
def known_secret_values():
    return sorted(set([value for value in (globals().get(key) for key in SECRET_KEYS) if isinstance(value, str) and len(value) >= MIN_REDACTABLE_SECRET_LENGTH and not value.startswith("your_")]) | set(_DELIVERY_SECRET_VALUES.get()), key=len, reverse=True)
...
def classify_recovery_error(error=None, context="runtime", detail=""):
    ...
    if transport_code:
        return advice(transport_code, "The Riot API did not answer in time" if transport_code == "network.timeout" else "The Riot API could not be reached", "Usually nothing to do, the tool retries on its own. If it continues, check network access, DNS, firewall and proxy settings", True, CONNECTION_GUIDE_URL)
    if "timed out" in message or "timeout" in message:
...
def outage_missed_alert_body(target, lasted, summary):
    return f"Monitoring failed for {target} at {get_date_from_ts(int(time.time()) - lasted)} and recovered after {display_time(lasted)}.\n\nThe failure was: {summary}\n\nThe failure alert could not be delivered here while the failure lasted."


# Builds the HTML body of the combined failure and recovery alert, matching the plain text
def outage_missed_alert_body_html(target, lasted, summary):
    return f"Monitoring failed for <b>{html_text(target)}</b> at <b>{html_text(get_date_from_ts(int(time.time()) - lasted))}</b> and recovered after <b>{html_text(display_time(lasted))}</b>.<br><br>The failure was: {html_text(summary)}<br><br>The failure alert could not be delivered here while the failure lasted."
...
_DATE_RANGE_RE = re.compile(r"\b(?:" + _WEEKDAY_ABBR_PATTERN + r")[\t ]\d{1,2}\s+\w{3}(?:\s+\d{2,4})?[\s,]*\d{2}:\d{2}(:\d{2})?(\s*[AP]M)?\s*-\s*\d{2}:\d{2}(:\d{2})?(\s*[AP]M)?\b", re.IGNORECASE)
...
def send_outage_recovery_alert(target, lasted, error_alert):
    ...
    email_text, email_html = (outage_missed_alert_body, outage_missed_alert_body_html) if email_missed else (outage_recovery_alert_body, outage_recovery_alert_body_html)
    webhook_text, webhook_html = (outage_missed_alert_body, outage_missed_alert_body_html) if webhook_missed else (outage_recovery_alert_body, outage_recovery_alert_body_html)
    send_notification_channels("error", subject, email_body_with_timestamp(email_text(target, lasted, error_alert.summary)), email_html_with_timestamp(email_html(target, lasted, error_alert.summary)), email_enabled=email_due or email_missed, webhook_enabled=webhook_due or webhook_missed, webhook_body=webhook_text(target, lasted, error_alert.summary), webhook_body_html=webhook_html(target, lasted, error_alert.summary))
    return True
...
def doctor_output_destination_checks(riot_id=None):
    ...
    if DISABLE_LOGGING:
        ...
        if suffix or Path(os.path.expanduser(str(LOL_LOGFILE))).suffix:
            checks.append(doctor_destination_check("Log destination", build_log_path(LOL_LOGFILE, suffix), creates_parents=True))
        else:
...
def command_reports_configuration(args=None):
    ...
    return any(getattr(args, name, False) for name in ("doctor", "setup", "set_riot_api_key", "set_smtp_password", "set_webhook_url"))
...
def doctor_check_configuration(config_path=None, env_path=None, riot_id=None, region=None):
    ...
    if finite_number(LOL_CHECK_INTERVAL) and isinstance(LOL_ACTIVE_CHECK_INTERVAL, (int, float)) and not isinstance(LOL_ACTIVE_CHECK_INTERVAL, bool) and 0 < LOL_ACTIVE_CHECK_INTERVAL < DOCTOR_MIN_SAFE_ACTIVE_INTERVAL:
...
def run_setup_wizard(initial_riot_id=None, initial_region=None, config_file=None, env_file=None, input_func=None, getpass_func=None, interactive=None):
    ...
    try:
        if doctor_offered and _wizard_ask_yes_no("Run doctor now? It writes no files and offers real delivery tests only with separate approval.", default=True, input_func=input_func):
            ...
            doctor_exit = run_doctor(riot_id=state.riot_id, region=state.region, config_path=str(state.config_path), env_path=str(state.env_path) if state.env_path.is_file() else None)
    except (EOFError, KeyboardInterrupt):
    ...
    if start_monitoring:
        ...
        launch_arguments = _wizard_local_command_args(riot_id=launch_riot_id, region=None if state.persist_target else state.region, config_path=state.config_path, env_path=state.env_path if state.env_path.is_file() else None)
        sys.stdout.flush()
...
def format_startup_summary_row(row):
    ...
    prefix = f"* {indent}{(row.label + ':'):<{STARTUP_SUMMARY_LABEL_WIDTH - len(indent)}}"
